...
resultsIB <- kfold.BN(year.folds.list = year.folds.list, mi.threshold = mi.threshold, BNB.args.list = BNB.args.list, 
                    plot_.DBN = plot_.DBN, plot.aucS = plot.aucS, plot.MI = plot.MI,
                    reference = reference, only.loes.third = only.loes.third)

resIB <- list(results = results, args = BNB.args.list)
save(resIB, file="CV/resCVIB.RData") 